...
# MOS Custom - make sure log file exists
[ ! -d /var/log/fsck ] && \
...
do_start () {
  ...
  if is_fastboot_active ; then
    [ "$fscheck" = yes ] && \
      log_warning_msg "Fast boot enabled, so skipping file system check."
    fscheck=no
  fi
...
Continuing with system boot in 5 seconds."
    ...
    if [ "$VERBOSE" = no ] ; then
      # MOS Custom - setterm --msg not supported in MOS
      log_action_begin_msg "Checking file systems"
      logsave_best_effort fsck $spinner -T -M -A $fix $force $FSCKTYPES_OPT >/dev/null 2>&1
      FSCKCODE=$?
